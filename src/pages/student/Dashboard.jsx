...
export default function Dashboard() {
  ...
  return <>
    ...
    <section className="import-results-card" style={{ margin: '0 0 24px', padding: '22px 24px', border: '1px solid var(--border, rgba(255,255,255,.12))', borderRadius: '18px', background: 'var(--panel, rgba(255,255,255,.04))', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '18px', flexWrap: 'wrap' }}>
      <div style={{ display: 'flex', alignItems: 'flex-start', gap: '14px', flex: '1 1 420px' }}>
        <span style={{ width: '42px', height: '42px', borderRadius: '12px', display: 'grid', placeItems: 'center', flex: '0 0 auto', background: 'rgba(255,255,255,.08)' }}><Icon name="upload" size={20} /></span>
        <div><strong style={{ display: 'block', marginBottom: '5px' }}>Already have your results in Excel?</strong><p style={{ margin: 0, opacity: .72, lineHeight: 1.5 }}>Skip manual entry. Upload your spreadsheet and CGPA+ will read the course, credit unit and grade data, show you a preview, then save the results you approve.</p></div>
      </div>
      <Button href="#/app/import" icon="upload">Import from Excel</Button>
    </section>
