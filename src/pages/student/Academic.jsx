...
export function ResultEditor({ open, onClose, record, onSaved }) {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      setForm(record
        ...
        : { ...blank, ...defaults, sessionId: defaults.sessionId || FALLBACK_SESSIONS.find(x => x.name === '2026/2027')?.id || FALLBACK_SESSIONS[FALLBACK_SESSIONS.length - 1].id, semesterId: defaults.semesterId || FALLBACK_SEMESTERS[0].id, levelId: defaults.levelId || 'level-300' });
    })();
    return () => { active = false; };
  }, [open, record]);
  ...
  return <Modal open={open} onClose={onClose} title={record ? 'Edit academic result' : 'Add an academic result'} description="Enter your course details and grade. CGPA+ calculates the quality points automatically." wide>
...
function CoursePlanner({ onChanged }) {
  ...
  async function load() { try { const preferences = await authService.getPreferences(); setCourses(Array.isArray(preferences.plannedCourses) ? preferences.plannedCourses : []); } catch (e) { setError(e?.message || 'Unable to load your course list.'); } }
  useEffect(() => { load(); }, []);
  ...
  async function save(e) { e.preventDefault(); if (!form.code.trim() || !form.title.trim() || !form.credits) return; setBusy(true); setError(''); try { const next = editing ? courses.map(c => c.id === editing.id ? { ...c, ...form, code: form.code.trim().toUpperCase(), credits: Number(form.credits) } : c) : [...courses, { ...form, id: crypto.randomUUID(), code: form.code.trim().toUpperCase(), credits: Number(form.credits) }]; await authService.updatePreferences({ plannedCourses: next }); setCourses(next); setOpen(false); onChanged?.(next); setForm({ ...blankCourse }); } catch (e) { setError(e?.message || 'Unable to save this course.'); } finally { setBusy(false); } }
  async function remove(id) { setBusy(true); setError(''); try { const next = courses.filter(c => c.id !== id); await authService.updatePreferences({ plannedCourses: next }); setCourses(next); onChanged?.(next); } catch (e) { setError(e?.message || 'Unable to remove this course.'); } finally { setBusy(false); } }
  return <Panel title="My semester courses" description="Add the courses you are taking now. You can enter the result later when your grade is released." action={<Button icon="plus" onClick={() => start()}>Add course</Button>}>
    {error ? <p className="field-error" role="alert">{error}</p> : null}
    {courses.length ? <div className="table-scroll"><table><thead><tr><th>Course</th><th>Title</th><th>Credits</th><th>Actions</th></tr></thead><tbody>{courses.map(course => <tr key={course.id}><td><strong>{course.code}</strong></td><td>{course.title}</td><td>{course.credits}</td><td><button type="button" className="text-button" onClick={() => start(course)}>Edit</button>{' '}<button type="button" className="text-button" onClick={() => remove(course.id)} disabled={busy}>Remove</button></td></tr>)}</tbody></table></div> : <div><Notice>No courses added yet. Add your registered courses so your workspace is ready before results arrive.</Notice><Button variant="outline" onClick={() => start()}>Add my first course</Button></div>}
    <Modal open={open} onClose={() => setOpen(false)} title={editing ? 'Edit course' : 'Add semester course'} description="Add a course to your personal semester list."><form className="form-stack" onSubmit={save}><div className="form-grid"><Field label="Course code"><Input value={form.code} onChange={change('code')} placeholder="e.g. CSC 301" required /></Field><Field label="Course title"><Input value={form.title} onChange={change('title')} placeholder="e.g. Artificial Intelligence" required /></Field><Field label="Credit units"><Input value={form.credits} onChange={change('credits')} type="number" min="1" step="1" required /></Field></div><Notice>You can add the grade/result later. This course list is private to your account.</Notice><div className="modal-actions"><Button variant="outline" type="button" onClick={() => setOpen(false)}>Cancel</Button><Button type="submit" busy={busy}>{editing ? 'Save course' : 'Add course'}</Button></div></form></Modal>
  </Panel>;
}
...
export default function Academic() {
  ...
  const semesterLoader = () => loadOrFallback(() => academicService.getSemesters(), FALLBACK_SEMESTERS);
  const levelLoader = () => loadOrFallback(() => academicService.getLevels(), FALLBACK_LEVELS);
  return <><PageHeader eyebrow="A HOME FOR EVERY RESULT" title="Your academic record." description="Add the courses you are taking, record your results when they are released, and let CGPA+ keep your GPA and CGPA organized." actions={<Button icon="plus" onClick={() => { setEditing(null); setEditor(true); }}>Add result</Button>} />
    ...
    <DataTable columns={resultColumns} resource={resource} search={search} emptyTitle="No results added yet." emptyDescription="When your result is released, add it here and CGPA+ will calculate the quality points for you." onView={setViewing} onEdit={record => { setEditing(record); setEditor(true); }} onDelete={setDeleting} />
    <p className="table-help">Tip: Add your courses first. When a result is released, tap <strong>Add result</strong>, choose the session, semester and level, then select your grade.</p>
    <ResultEditor open={editor} onClose={() => setEditor(false)} record={editing} onSaved={refresh} />
