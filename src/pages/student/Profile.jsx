...
import { safeExternalUrl } from '../../utils/files';
...
export function AcademicProfileEditor({ open, onClose, profile, onSaved }) {
  ...
  const action = useAction();
  useEffect(() => {
    if (open) setForm(Object.fromEntries(Object.keys(emptyAcademic).map(key => [key, profile?.[key] ?? emptyAcademic[key]])));
  }, [open, profile]);
