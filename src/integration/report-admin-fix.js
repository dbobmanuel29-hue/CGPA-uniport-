...
async function adminPreview(request) {
  ...
  if (request.reportId) {
    ...
    if (!snap.exists) throw Object.assign(new Error('Report not found.'), { code: 'not-found' });
  ...
  if (type === 'academic' || type === 'semester') {
    ...
    return { id: null, studentName: user.fullName || user.email || 'Student', sessionName: type === 'semester' ? 'Semester report' : 'Academic record', createdAt: now(), matriculationNumber: profile.matriculationNumber || '', summary: summary(courses), courses };
  }
  if (type === 'support') {
