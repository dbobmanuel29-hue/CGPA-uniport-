...
async function sdk() {
  ...
  if (!value) throw fail('Firebase is not configured.', 'BACKEND_NOT_CONNECTED');
  return value;
}
async function requireUser() {
  const value = await sdk();
  if (!value.auth.currentUser) throw fail('Authentication required.', 'unauthorized');
  return value;
}
async function requireAdmin() {
  ...
  if (user.uid !== OWNER_ADMIN_UID) {
    ...
    if (token.claims.admin !== true && token.claims.role !== 'admin') throw fail('Administrator access required.', 'permission-denied');
  }
  return value;
}
...
async function findStudent(db, matriculationNumber) {
  ...
  if (!wanted) throw fail('Enter the student matriculation number.', 'validation/student-id');
  try {
    let profile = null;
    for (const field of ['matriculationNumber', 'matricNo', 'matNumber']) {
      const snap = await db.collection('academicProfiles').where(field, '==', String(matriculationNumber).trim()).limit(1).get();
      if (!snap.empty) { profile = { id: snap.docs[0].id, ...snap.docs[0].data() }; break; }
    }
    if (!profile) {
      const profiles = rows(await db.collection('academicProfiles').get());
      profile = profiles.find(item => normalise(item.matriculationNumber || item.matricNo || item.matNumber) === wanted) || null;
    }
    if (!profile) {
      const users = rows(await db.collection('users').get());
      const userMatch = users.find(item => normalise(item.matriculationNumber || item.matricNo || item.matNumber) === wanted);
      if (userMatch) return { user: userMatch, profile: { id: userMatch.id, matriculationNumber: userMatch.matriculationNumber || userMatch.matricNo || userMatch.matNumber } };
      throw fail(`No student was found with matriculation number “${matriculationNumber}”.`, 'not-found/student');
    }
    const uid = profile.userId || profile.id;
    const userSnap = await db.collection('users').doc(uid).get();
    if (!userSnap.exists || userSnap.data()?.accountStatus === 'deleted') throw fail('The student account could not be found.', 'not-found/student');
...
async function resultRows(db, uid, request = {}) {
  try {
    ...
    throw fail(`The student's results could not be loaded. Firebase reported: ${error?.message || 'unknown error'}`, 'report/results-failed');
  }
}
...
async function adminPreview(request) {
  ...
  if (request.reportId) {
    const snap = await db.collection('reports').doc(request.reportId).get();
    if (!snap.exists) throw fail('Report not found.', 'not-found/report');
    return { ...(snap.data().preview || {}), id: snap.id };
  }
  ...
  if (type === 'academic' || type === 'semester') {
    ...
    return { id: null, studentName: user.fullName || user.email || 'Student', sessionName: type === 'semester' ? 'Semester report' : 'Academic record', createdAt: now(), matriculationNumber: profile.matriculationNumber || profile.matricNo || profile.matNumber || '', summary: summary(courses), courses };
  }
  if (type === 'support') {
...
async function reportPreview(request = {}) {
  ...
  const results = await resultRows(db, auth.currentUser.uid, request);
