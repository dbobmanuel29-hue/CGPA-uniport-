    ...
    auth: {
      deleteAccount: async () => {
        ...
        if (user.uid === OWNER_ADMIN_UID) {
          throw Object.assign(
            new Error('The owner administrator account cannot be deleted from student settings.'),
            { code: 'auth/owner-account-protected' }
          );
        }

        // Remove all records owned by the student before deleting the
        // Firebase Authentication account. This keeps deletion Spark-safe
        // and prevents the account from leaving its academic data behind.
