...
async function enrichAcademic(db, profile) {
  ...
  await Promise.all(relations.map(async ([field,collection,target]) => { if (!profile[field]) return; const snap=await db.collection(collection).doc(profile[field]).get(); if(snap.exists) output[target]=snap.data()?.name||snap.data()?.title||snap.data()?.label||''; }));
  return output;
}
...
export async function registerAdminFixes() {
  const methods = {
    async getStudents(filters = {}) {
      ...
      if(filters.status)output=output.filter(x=>x.accountStatus===filters.status); if(filters.facultyId)output=output.filter(x=>x.facultyId===filters.facultyId); return output;
    },
    async getStudent(studentId) {
      const { db } = await requireAdmin();
      const userSnap=await db.collection('users').doc(studentId).get(); if(!userSnap.exists)throw Object.assign(new Error('Student not found.'),{code:'not-found'});
      const profileSnap=await db.collection('academicProfiles').doc(studentId).get(); const resultSnap=await db.collection('results').where('userId','==',studentId).get(); const ticketSnap=await db.collection('supportTickets').where('userId','==',studentId).get();
      const profile=profileSnap.exists?await enrichAcademic(db,profileSnap.data()):null; const results=rows(resultSnap).map(r=>({...r,createdAt:cleanDate(r.createdAt),updatedAt:cleanDate(r.updatedAt)})); const effective=effectiveResults(results); const summary=calculate(effective); const semesterMap=new Map();
      for(const result of results){const key=`${result.levelId||'unknown'}:${result.semesterId||'unknown'}`;const current=semesterMap.get(key)||[];current.push(result);semesterMap.set(key,current);}
      ...
      return {id:userSnap.id,...userSnap.data(),createdAt:cleanDate(userSnap.data().createdAt),academicProfile:profile,results,summary,gpaHistory,tickets:rows(ticketSnap).map(t=>({...t,createdAt:cleanDate(t.createdAt)}))};
    },
  };
