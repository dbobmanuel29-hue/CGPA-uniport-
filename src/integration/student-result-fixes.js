...
function normalizePayload(payload) {
  ...
  const attemptType = payload?.attemptType === 'carryover' ? 'carryover' : 'regular';
  const originalCourseCode = attemptType === 'carryover'
    ? String(payload?.originalCourseCode || code).trim().toUpperCase()
    : '';

  if (!code || !title || !grade) {
    throw Object.assign(new Error('Course code, course title and grade are required.'), { code: 'validation/invalid-result' });
  }
  if (!Number.isFinite(credits) || credits <= 0) {
    throw Object.assign(new Error('Credit units must be a valid number greater than 0.'), { code: 'validation/invalid-credits' });
  }
  if (!Number.isFinite(points) || points < 0 || points > 5) {
    throw Object.assign(new Error('Grade point must be a valid number from 0 to 5.'), { code: 'validation/invalid-grade-point' });
  }
  if (!sessionId || !semesterId || !levelId) {
    throw Object.assign(new Error('Academic session, semester and level are required.'), { code: 'validation/incomplete-academic-context' });
  }
...
function friendlyError(error) {
  if (error?.code === 'permission-denied') {
    return Object.assign(
      new Error(
        'Firebase rejected this result. Check that Student result entry is ON in Admin Settings → Academic Configuration, and that your email is verified if email verification is required in Security settings.'
      ),
      { code: 'academic/result-permission-denied', cause: error }
    );
  }
  if (error?.code === 'unavailable') {
    return Object.assign(new Error('Firebase is temporarily offline. Check your internet connection and try again.'), { code: error.code });
  }
  if (error?.code === 'failed-precondition') {
    return Object.assign(new Error('Firebase needs a database/index setup before this result can be saved.'), { code: error.code });
  }
  if (error?.code === 'invalid-argument') {
    return Object.assign(new Error('One of the result fields contains an invalid value. Check the credit units and grade.'), { code: error.code });
  }
  return error;
}
      ...
      updateResult: async (resultId, payload) => {
        ...
        try {
          ...
          if (!snap.exists || snap.data().userId !== user.uid) {
            throw Object.assign(new Error('Result not found.'), { code: 'not-found' });
          }
