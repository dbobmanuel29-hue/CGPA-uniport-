...
function filterRows(rows, filters = {}) {
  return rows.filter(row =>
    ...
    (!filters.semesterId || row.semesterId === filters.semesterId) &&
    (!filters.levelId || row.levelId === filters.levelId)
  );
}
...
function semesterSeries(results) {
  ...
  return [...groups.values()].map(rows => {
    ...
    return {
      ...
      levelName: rows[0]?.levelName || rows[0]?.levelId || '',
      gpa: summary.gpa,
      cgpa: cumulativeCredits ? cumulativeQuality / cumulativeCredits : 0,
  ...
  configureServices({
    academic: {
      ...
      getSummary: async filters => {
        const results = filterRows(await readResults(), filters);
        const all = await readResults();
        return { ...calculate(results), cgpa: calculate(all).cgpa };
      },
      getAnalytics: async () => {
        const results = await readResults();
        const summary = calculate(results);
        const series = semesterSeries(results);
        const grades = {};
        results.forEach(row => { const grade = row.grade || 'Unknown'; grades[grade] = (grades[grade] || 0) + 1; });
        const gpas = series.map(row => row.gpa);
        const failed = results.filter(row => Number(row.points) === 0).length;
        return {
          maxPoint: 5,
          stats: {
            cgpa: summary.cgpa,
            ...
            averageGpa: gpas.length ? gpas.reduce((sum, value) => sum + value, 0) / gpas.length : 0,
            totalCredits: summary.totalCredits,
            failedCourses: failed,
            repeatedCourses: 0,
            bestSemester: [...series].sort((a, b) => b.gpa - a.gpa)[0]?.label || '--',
            weakestSemester: [...series].sort((a, b) => a.gpa - b.gpa)[0]?.label || '--',
          },
          series,
          grades: Object.entries(grades).map(([label, value]) => ({ label, value })),
      ...
      getDashboard: async () => {
        ...
        const summary = calculate(results);
        const current = profile?.currentSessionId && profile?.currentSemesterId
          ? results.filter(row => row.sessionId === profile.currentSessionId && row.semesterId === profile.currentSemesterId && (!profile.currentLevelId || row.levelId === profile.currentLevelId))
          : [];
        const currentSummary = calculate(current);
        let notifications = [];
        try {
          const sdk = await getFirebase();
          const snap = await sdk.db.collection('notifications').where('userId', '==', sdk.auth.currentUser.uid).get();
          notifications = snap.docs.map(doc => ({ id: doc.id, ...doc.data(), createdAt: dateValue(doc.data().createdAt) })).sort((a, b) => String(b.createdAt).localeCompare(String(a.createdAt)));
        } catch {}
        return {
          summary: { ...summary, classification: summary.cgpa >= 4.5 ? 'First Class' : summary.cgpa >= 3.5 ? 'Second Class Upper' : summary.cgpa >= 2.4 ? 'Second Class Lower' : summary.cgpa >= 1.5 ? 'Third Class' : summary.cgpa >= 1 ? 'Pass' : 'Awaiting result data' },
          gpaTrend: analytics.series,
          cgpaProgressPercent: (summary.cgpa / 5) * 100,
          academicProgress: { percent: null, completedCredits: summary.totalCredits, remainingCredits: null },
          currentSemester: profile ? { sessionName: profile.currentSessionName || '--', semesterName: profile.currentSemesterName || '--', levelName: profile.currentLevelName || '--', courseCount: current.length, gpa: currentSummary.gpa } : null,
          recentResults: [...results].sort((a, b) => String(b.createdAt).localeCompare(String(a.createdAt))).slice(0, 5),
