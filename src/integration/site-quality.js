...
export function initSiteQuality() {
  const update = () => {
    ...
    let description = descriptions[path] || 'CGPA+ UniPort is an independent academic planning companion for University of Port Harcourt students.';
    let meta = document.querySelector('meta[name="description"]');
    if (!meta) { meta = document.createElement('meta'); meta.name = 'description'; document.head.appendChild(meta); }
    meta.content = description;
    let canonical = document.querySelector('link[rel="canonical"]');
    if (!canonical) { canonical = document.createElement('link'); canonical.rel = 'canonical'; document.head.appendChild(canonical); }
    canonical.href = `https://cgpa-lovat-theta.vercel.app/${path === '/' ? '' : `#${path}`}`;
    ...
    setMeta('twitter:card', 'summary_large_image'); setMeta('twitter:title', title); setMeta('twitter:description', description); setMeta('twitter:image', 'https://cgpa-lovat-theta.vercel.app/social-preview.svg');
  };
  update();
