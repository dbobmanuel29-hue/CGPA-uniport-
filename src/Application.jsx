...
function Router() {
  ...
  useEffect(() => {
    ...
    if (studentProtected || path.startsWith('/admin') || path === '/onboarding') {
      if (!session.user) { navigate('/login'); return; }
      if (path.startsWith('/admin') && session.user.role !== 'admin') navigate('/app');
    }
  }, [path, session.user, session.status, studentProtected]);
  useRevealObserver();
  ...
  if (path === '/app' || path.startsWith('/app/')) {
    if (publicCalculator && !session.user) return <PublicLayout path={path}><ReadingProgress />{page}</PublicLayout>;
    return <WorkspaceLayout path={path}>{page}</WorkspaceLayout>;
  }
