...
const headerScore = (header, names) => {
  ...
  return names.reduce((best, name) => {
    ...
    return h === n ? 100 : h.includes(n) || n.includes(h) ? Math.max(best, 60) : best;
  }, 0);
};
function detectMapping(headers) {
  return Object.fromEntries([...REQUIRED, 'grade', 'points', 'score', 'levelId', 'sessionId', 'semesterId'].map(key => {
...
function makeRows(sheetRows, mapping, context) {
  return sheetRows.map((row, index) => {
    ...
    return { _row: index + 2, code, title, credits, grade, points: option?.points ?? pointsRaw, sessionId: sessionIdFromValue(get('sessionId')) || context.sessionId || '', semesterId: semesterIdFromValue(get('semesterId')) || context.semesterId || '', levelId: levelIdFromValue(get('levelId')) || context.levelId || '', score };
...
export default function ExcelResultImporter({ context = {}, onImported, compact = false }) {
  ...
  useEffect(() => { if (rawRows.length) setRows(makeRows(rawRows, mapping, context)); }, [rawRows, mapping, context]);
  ...
  async function readFile(file) {
    ...
    try {
      ...
      setHeaders(Object.keys(best.rows[0])); setMapping(detected); setRawRows(best.rows); setMessage(`${best.rows.length} spreadsheet rows found from “${best.name}”.`);
